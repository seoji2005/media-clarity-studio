...
| 문서 기반 | 초안 → **독립 리뷰 완료 → 지적 반영 완료** |
| 열린 PR | **#1** (Draft) — 병합되지 않음 |
| 다음 작업 | 사람 오너 답변 대기 + TASK-002/003 착수 가능 |
| 차단 요인 | 사람 오너의 답변 **2건** (§5) |
...
| [TASK-002](docs/tasks/TASK-002.md) | REVIEW-001 지적 반영 | Claude | Codex | 0 | **In review** (이 PR의 후속 커밋) |
...
### 3.2 독립 리뷰 (TASK-001, Codex)

- 대상 커밋 `38146ae`, 판정 **변경 요청**
- 차단 5건 · 중대 15건 · 경미 2건 → [`docs/reviews/REVIEW-001.md`](docs/reviews/REVIEW-001.md)
- 출처: Codex/GPT Work가 수행, 사람 제품 오너를 통해 전달, Claude가 전사

### 3.3 리뷰 반영 (TASK-002, Claude — 이 PR의 후속 커밋)
...
| 후보 | 내용 | Owner 제안 |
|---|---|---|
| **TASK-003** | **seed 코퍼스·라이선스·합성 데이터 대안 조사 + 비교표 작성** | 미정 |
| TASK-004 | REVIEW-001 반영에 대한 2차 독립 검토 | **Codex** (Claude가 반영했으므로) |
...
| ID | 질문 | 왜 막히는가 |
|---|---|---|
| **U-08** | 채점 정답 자막은 **원어**인가 **번역**인가? | 번역이면 파이프라인에 단계가 추가되어 구조가 달라짐 |
| **U-11** | 과제 제출 기한은 언제인가? | 일정 계획과 각 도메인의 목표 수준 결정 |
...
| R-7 | 에이전트 간 문서 갈라짐 | 서로 다른 전제로 작업 | `AGENTS.md` 단일 출처 (ADR-0008) |
...
| **R-10** | **반영이 형식적일 수 있음** | 구조만 만들고 실효 없음 | 2차 독립 검토(TASK-004 후보)로 확인 |

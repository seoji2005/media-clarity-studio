...
마지막 갱신: 2026-09-04 (TASK-032 typed preparation evidence slice 검증 완료, Gate H review 대기)
...
| 현재 작업 | **TASK-032** — typed configuration·access/model/dependency receipt와 pack·decision rule·Work CPU evidence의 CAS/identity 교차 검증, source-group 누출·ref 교환·inventory/platform 위조 반례 작성·검증 완료; fresh Gate H review 대기. 실제 값·모델 실행은 미착수 |
| U-31 | **답변됨 (2026-08-22)** — 번역 대상 언어 **한국어(`ko`)** |
| 다음 실행 의존성 | 현재 preparation fixed HEAD의 fresh Gate H review와 제품 오너 merge 결정. 그 뒤 실제 source/config 값, dependency lock과 pack/model/access receipt를 첫 candidate output 전에 고정해야 함. 실제 target-hardware 판정은 Windows 접근 뒤 별도 evidence가 필요 |
...
| [TASK-032](docs/tasks/TASK-032.md) | 공개·합성 frozen pack 3-ASR Work-CPU screening | Lean Root Author | fresh GPT/Codex Gate H reviewer | Phase 1a ASR candidate screening | **In progress — typed preparation evidence slice 작성·검증 완료, fresh Gate H review 대기**; 실제 model/dependency/network 실행과 TASK-031 후보·8/12 계약 변경 없음 |
...
| [TASK-032](docs/tasks/TASK-032.md) | 공개·허가·합성 frozen pack 3-ASR Work-CPU screening — typed preparation evidence slice 작성·검증 완료, fresh Gate H review 대기; 실제 값·모델·실제 pack·network 실행 미착수 | Lean Root Author |

...
마지막 갱신: 2026-08-30 (TASK-029 자막 spine 계약 구현 — REVIEW-023 변경 요청 반영, 새 고정 HEAD 검토 대기)
...
| 현재 작업 | **TASK-029 새 고정 HEAD Gate H 검토 대기** — `claude/task-029-subtitle-spine-contracts`에 schema 5개·domain validator·fixture 104건·mutation 감사 구현 완료, [`REVIEW-023`](docs/reviews/REVIEW-023.md) 변경 요청 반영(TASK-029 §15). 구현 세션 자기 승인 없음 |
...
| [TASK-029](docs/tasks/TASK-029.md) | SpeechSegment·Transcript·capability·translation·subtitle 실행 계약 정본화 | Claude Code 구현 세션 | Lean Root 고정 HEAD Gate H 검토 | Phase 1a subtitle data spine | **In review** — 계약 병합(PR #44) 후 구현 완료, `REVIEW-023` 변경 요청을 같은 브랜치에 반영(TASK-029 §15), `Implemented — awaiting fixed HEAD review` (구현 세션 자기 승인 없음) |
...
| TASK | 내용 | Owner |
|---|---|---|
| [TASK-029](docs/tasks/TASK-029.md) — 새 고정 HEAD 검토 대기 | SpeechSegment·Transcript·ASR capability·TranslatedTranscript·SubtitleDocument의 기계 정본 5개, domain validator, fixture 104건, input/schema/validator-code mutation 감사 + 민감 값 비노출 스캔·방어면 coverage guard (REVIEW-023 반영) | 구현: Claude Code → 검증: Lean Root (Gate H) |
...
| **TASK-029** | 자막 spine 기계 계약 정본화 | TASK-028 완료와 별도 TASK 계약 | **In review — REVIEW-023 변경 요청 반영, 새 고정 HEAD Gate H 검토 대기** |

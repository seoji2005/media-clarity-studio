...
마지막 갱신: 2026-08-28 (TASK-028 REVIEW-019 반영 — 고정 HEAD Gate H 재검토 대기)
현재 단계: **Phase 1a** — TASK-028 REVIEW-019 반영 완료, 고정 HEAD Gate H 재검토 대기
...
| 병합된 PR | **#1·#5·#16·#18·#19·#21·#22·#23·#24·#25·#28·#33·#34** — 각 merge commit은 §3에 기록; 현재 HEAD는 GitHub에서 조회 |
...
| 현재 작업 | **TASK-028 고정 HEAD Gate H 재검토 대기** — `claude/task-028-resumable-runtime`에 구현 완료 후 `REVIEW-018` (PR #37) M-01~M-05와 `REVIEW-019` (PR #38) M-01-R1~R3·M-05-R1 반영. 구현 세션 자기 승인 없음 |
...
| 차단 요인 | **TASK-028 구현 착수 차단 없음.** U-16 보관 정책은 미정이므로 자동 삭제·GC를 범위 밖에 두며, 다른 U-XX도 임의 확정하지 않음 |
...
| [TASK-028](docs/tasks/TASK-028.md) | Content-addressed artifact store와 재개 가능한 stage runtime | Claude Code 구현 세션 | Lean Root 고정 HEAD Gate H 검토 | Phase 1a shared storage·orchestrator foundation | **In review** — 1차 `REVIEW-018` (PR #37) · 2차 `REVIEW-019` (PR #38) **변경 요청** 반영, 구현 `Implemented — awaiting fixed HEAD rereview` (구현 세션 자기 승인 없음) |
...
- 계약만 승인·병합됐으며 구현 코드·schema·테스트·dependency·CI는 아직 없다. TASK 상태는 `Not started`다.
...
| TASK | 내용 | Owner |
|---|---|---|
| [TASK-028](docs/tasks/TASK-028.md) — 고정 HEAD 재검토 대기 | content-addressed artifact store, stage fingerprint/cache, crash-safe checkpoint와 resume 기반. J-01~J-16 16건, mutation 감사, `REVIEW-018`·`REVIEW-019` 반영 포함 | Claude Code 구현 → Lean Root 검증 (Gate H) |
...
### Phase 1a 실행 순서 **(전체 후속 순서는 제안됨 유지; TASK-028 계약은 승인됨.** [`PLAN.md`](PLAN.md) §3-1d**)**
...
| **TASK-028** | 공용 artifact/cache/resume runtime | TASK-006 완료 | **In review — REVIEW-018·019 반영, 고정 HEAD 재검토 대기** |
...
> TASK-028 계약은 PR #34로 승인·병합됐습니다. TASK-022의 원본 불변·partial·no-overwrite 경험을
> 공용 storage/orchestrator 기반으로 일반화하는 구현은 Claude Code가 수행하고 Lean Root가 고정 HEAD에서 검증합니다.
> 모델·corpus·ASR·번역·metric·worker supervision은 이 TASK에 포함하지 않습니다.

...
| ID | 답변 | 남은 것 |
|---|---|---|
| **U-08** | **채점 정답은 번역 자막** | **번역 대상 언어 미정 → U-31 신규 등록** |
...
|---|---|---|---|
| **TASK-003** | seed 코퍼스·라이선스·합성 데이터 대안 조사 + 비교표 작성 | **없음 — 지금 착수 가능** | **아니오** |
| **(사람) U-06 선택** | 제품 오너가 조사 비교표를 보고 선택 | TASK-003 결과 | 아니오 |
| **(사람) U-31 답변** | 번역 대상 언어 — **독립된 게이트** | **없음 — 언제든 답할 수 있음** | — |
| **TASK-005** | 평가 하네스 설계 명세 (Phase 1a) | **TASK-003 · U-06 선택 · U-31 답변 (셋 모두)** | **예** |
| **TASK-006** | `ReferenceBundle/v1` 구체화 | **TASK-005** | 예 (TASK-005를 통해) |
| 코드 구현 | — | TASK-006까지 완료 | 예 |

> **다음 실행 후보는 TASK-003입니다.** ADR-0024에 따라 **에이전트가 조사**하고, 오너는
> 비교표를 보고 고르기만 합니다. 이것이 U-06을 차단 항목에서 뺀 이유입니다.
...
**아직 남은 것 — 두 가지를 섞지 마십시오**

| 항목 | 무엇이 막히는가 | 누가 답하나 |
|---|---|---|
| **정확한 제출 날짜** (U-11의 미답변 부분) | **정밀 일정 수립.** 마일스톤·버퍼를 **날짜로 고정**하는 것 | 사람 제품 오너 |
| **U-31 — 번역 대상 언어** | **TASK-005부터.** TASK-005의 선행 조건이며 TASK-006도 그 뒤입니다. **TASK-003은 막지 않습니다** (§4) | 사람 제품 오너 |
...
| ID | 질문 | 왜 중요한가 |
|---|---|---|
| **U-31** | **번역 대상 언어는 무엇인가** (U-08 답변에서 파생) | **Phase 1 번역 단계 설계 전 필요.** 추측하지 않음 |

...
**[`AGENTS.md`](AGENTS.md)가 규칙의 단일 출처입니다. 작업 전에 반드시 읽으세요.**
이 파일은 그 내용을 반복하지 않습니다. 충돌 시 `AGENTS.md`가 이깁니다.

## Claude가 자주 잊는 것 세 가지

1. **`main`에 쓰지 않는다.** 항상 `claude/<slug>` 브랜치 → PR.
2. **"복원"이라고 쓰지 않는다.** 재구성(reconstruction) / 추정(estimation)만 사용. (`AGENTS.md` §1)
...
- 새 아키텍처 판단은 [`docs/DECISIONS.md`](docs/DECISIONS.md)에 **제안됨/승인됨/미해결** 라벨과 함께 기록
